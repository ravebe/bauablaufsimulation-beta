// DatePicker.tsx — Kalender-Popup im TC-Design
...
export default function DatePicker({ value, onChange, label }: Props) {
  ...
  // Kalender öffnen → zum aktuellen Datum springen
  useEffect(() => {
  ...
  function waehlen(tag: number) {
    onChange(fmtDMY(new Date(jahr, monat, tag)));
    setOffen(false);
  }
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-flex", alignItems: "center" }}>
      {label && <span style={{ fontSize: 10, color: "#8a9baa", marginRight: 4 }}>{label}</span>}
      <button
        onClick={e => { e.stopPropagation(); setOffen(o => !o); }}
        style={{ background: "none", border: "none", cursor: "pointer", padding: "2px 4px", fontSize: 12, color: "#2d7dbd", display: "flex", alignItems: "center", gap: 3 }}
        title="Kalender öffnen"
      >
        <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="#2d7dbd" strokeWidth="1.3">
          <rect x="1" y="3" width="14" height="12" rx="1.5" />
          <line x1="1" y1="7" x2="15" y2="7" />
          <line x1="4.5" y1="1" x2="4.5" y2="5" />
          <line x1="11.5" y1="1" x2="11.5" y2="5" />
        </svg>
      ...
      {offen && (
        <div style={{
          ...
          boxShadow: "0 4px 16px rgba(0,0,0,.35)", width: 220, padding: 0,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", textAlign: "center", padding: "0 8px 8px", gap: 1 }}>
            {zellen.map((tag, i) => {
              ...
              const istGew = parsed && d.toDateString() === parsed.toDateString();
              return (
                <button key={tag} onClick={() => waehlen(tag)} style={{
                  width: 28, height: 28, borderRadius: "50%", border: "none", cursor: "pointer",
                  fontSize: 12, fontWeight: istGew || istHeute ? 700 : 400,
                  background: istGew ? "#3a7bd5" : istHeute ? "rgba(255,255,255,.15)" : "transparent",
                  color: istGew ? "#fff" : istHeute ? "#90caf9" : "rgba(255,255,255,.85)",
                  outline: istHeute && !istGew ? "1.5px solid #90caf9" : "none",
                  margin: "auto",
                }}>{tag}</button>
              );
            })}
          ...
          {/* Heute Button */}
          <button onClick={() => waehlen(heute.getDate())} style={{
            display: "block", width: "100%", padding: "8px 0", background: "rgba(255,255,255,.08)",
            border: "none", borderTop: "1px solid rgba(255,255,255,.15)", color: "#90caf9",
            cursor: "pointer", fontSize: 12, fontWeight: 600, borderRadius: "0 0 6px 6px",
          }}>Heute</button>
        </div>
      )}
    </div>
