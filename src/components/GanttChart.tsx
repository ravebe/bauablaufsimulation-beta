...
export default function GanttChart({ tasks, currentTag, totalTage, minDate, onTaskClick, onSliderChange, selTaskId, selGuids, taskSort, height, editable, onDateChange }: Props) {
  ...
  const startBarDrag = useCallback((e: React.MouseEvent, taskId: string, mode: "start" | "end" | "move", origStart: Date, origEnd: Date) => {
    ...
    const sx = e.clientX;
    const oS = (origStart.getTime() - minDate.getTime()) / 86400000;
    const oE = (origEnd.getTime() - minDate.getTime()) / 86400000;
    const dur = oE - oS;
  ...
  if (taskSort === "datum") {
    sorted.sort((a, b) => { const sa = parseDateUniversal(a.task.start)?.getTime() ?? 0, sb = parseDateUniversal(b.task.start)?.getTime() ?? 0; return sa !== sb ? sa - sb : (parseDateUniversal(a.task.end)?.getTime() ?? sa) - (parseDateUniversal(b.task.end)?.getTime() ?? sb); });
  } else if (taskSort === "aktiv") {
    sorted.sort((a, b) => { const aH = selGuids?.size && a.task.objektGuids.some(g => selGuids.has(g)) ? 1 : 0; const bH = selGuids?.size && b.task.objektGuids.some(g => selGuids.has(g)) ? 1 : 0; return bH - aH; });
  }
      ...
      <div style={{ display: "flex", flexShrink: 0 }}>
        ...
        <div ref={headerRef} style={{ flex: 1, height: HEAD_H, overflow: "hidden", background: "#f5f7f9", borderBottom: "1px solid #d4dce4" }}>
          <svg width={chartW} height={HEAD_H} style={{ display: "block" }}>
            {hLabels.map((m, i) => (
              <g key={`h${i}`}><line x1={m.x} y1={0} x2={m.x} y2={HEAD_H} stroke="#d4dce4" strokeWidth={0.6} />
                <text x={m.x + 4} y={14} fontSize={11} fontWeight={600} fill="#555">{m.label}</text></g>
            ))}
        ...
        <div ref={bodyRef} onScroll={syncScroll} style={{ flex: 1, overflow: "auto", position: "relative" }}>
          <svg width={chartW} height={bodyH} style={{ display: "block" }}>
            ...
            {sorted.map(({ task: t }, i) => {
              ...
              const showDates = editable && sd && ed && pxProTag >= 2;
              return (
                <g key={t.id}>
                  ...
                  {showDates && <text x={bX - 3} y={y + ROW_H / 2 + 4} fontSize={8} fill="#8a9baa" textAnchor="end"
                    style={{ cursor: "pointer" }} onClick={(e) => {
                      e.stopPropagation();
                      const rect = (e.target as SVGElement).getBoundingClientRect();
                      setCalEdit({ taskId: t.id, field: "start", value: fmtDMY(sd!), x: rect.left, y: rect.bottom });
                    }}>{fmtShort(sd!)}</text>}
                  ...
                  {sd && bW > 28 && <text x={bX + 5} y={y + ROW_H - 8} fontSize={8} fill="rgba(255,255,255,.9)" fontWeight={600} style={{ pointerEvents: "none" }}>{dauer}d</text>}

                  {/* Ende-Datum nach Balken */}
                  {showDates && <text x={bX + bW + 3} y={y + ROW_H / 2 + 4} fontSize={8} fill="#8a9baa"
                    style={{ cursor: "pointer" }} onClick={(e) => {
                      e.stopPropagation();
                      const rect = (e.target as SVGElement).getBoundingClientRect();
                      setCalEdit({ taskId: t.id, field: "end", value: fmtDMY(ed!), x: rect.left, y: rect.bottom });
                    }}>{fmtShort(ed!)}</text>}
      ...
      {calEdit && onDateChange && (
        <div style={{ position: "fixed", left: calEdit.x, top: calEdit.y, zIndex: 300 }}>
          <DatePicker value={calEdit.value} onChange={(val: string) => {
            ...
            if (!t) return;
            if (calEdit.field === "start") onDateChange(t.id, val.split(".").reverse().join("-"), t.end);
            else onDateChange(t.id, t.start, val.split(".").reverse().join("-"));
            setCalEdit(null);
          }} />
