...
export default function GanttChart({ tasks, currentTag, totalTage, minDate, onTaskClick, onSliderChange, selTaskId, selGuids, taskSort }: Props) {
  ...
  useEffect(() => {
    if (!bodyScrollRef.current || totalTage <= 0) return;
    const viewW = bodyScrollRef.current.clientWidth;
  ...
  const syncVScroll = useCallback(() => {
    const body = bodyScrollRef.current;
    const labels = labelBodyRef.current;
    if (body && labels) labels.scrollTop = body.scrollTop;
  }, []);
  ...
  // Smart Monats-Labels
  const rawMarkers: { x: number; m: number; y: number }[] = [];
  for (let d = 0; d <= totalTage; d++) {
    const date = new Date(minDate.getTime() + d * 86400000);
    if (date.getDate() === 1) rawMarkers.push({ x: d * pxProTag, m: date.getMonth(), y: date.getFullYear() });
  }
  if (rawMarkers.length === 0 || rawMarkers[0].x > 20) {
    rawMarkers.unshift({ x: 0, m: minDate.getMonth(), y: minDate.getFullYear() });
  }

  // Überschneidungs-Check
  let mode: "full" | "short" | "year" = "full";
  if (rawMarkers.length > 1) {
    const avg = rawMarkers.reduce((s, m, i) => i > 0 ? s + (m.x - rawMarkers[i - 1].x) : s, 0) / (rawMarkers.length - 1);
    if (avg < 22) mode = "year";
    else if (avg < 48) mode = "short";
  }

  let headerLabels: { x: number; label: string; line: boolean }[] = [];
  if (mode === "year") {
    let lastY = -1;
    for (const m of rawMarkers) {
      if (m.y !== lastY) { headerLabels.push({ x: m.x, label: String(m.y), line: true }); lastY = m.y; }
    }
  } else if (mode === "short") {
    headerLabels = rawMarkers.map(m => ({ x: m.x, label: `${MONAT_KURZ[m.m]} ${String(m.y).slice(2)}`, line: true }));
  } else {
    headerLabels = rawMarkers.map(m => ({ x: m.x, label: `${MONAT_VOLL[m.m]} ${String(m.y).slice(2)}`, line: true }));
  }

  const nadelX = currentTag * pxProTag;

  return (
    <div style={{ display: "flex", flexDirection: "column", border: "1px solid #d4dce4", background: "#fff", overflow: "hidden", height: "100%" }}>
      {/* === HEADER ROW (fixiert) === */}
      <div style={{ display: "flex", flexShrink: 0 }}>
        {/* Header links: Task/Tage */}
        <div style={{ width: labelW, flexShrink: 0, height: HEAD_H, background: "#f5f7f9", borderBottom: "1px solid #d4dce4", borderRight: "1px solid #d4dce4",
          ...
          <div onMouseDown={startLabelResize} style={{ position: "absolute", top: 0, right: -3, width: 6, height: "100%", cursor: "col-resize", zIndex: 5 }} />
        </div>
        {/* Header rechts: Zeitachse */}
        <div ref={headerScrollRef} style={{ flex: 1, height: HEAD_H, overflow: "hidden", background: "#f5f7f9", borderBottom: "1px solid #d4dce4" }}>
          <svg width={chartW} height={HEAD_H} style={{ display: "block" }}>
            {headerLabels.map((m, i) => (
              <g key={i}>
                {m.line && <line x1={m.x} y1={0} x2={m.x} y2={HEAD_H} stroke="#d4dce4" strokeWidth={0.6} />}
                <text x={m.x + 4} y={19} fontSize={11} fontWeight={600} fill="#555">{m.label}</text>
              </g>
            ))}
            {/* Nadel im Header */}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        {/* Body links: Task-Labels (fixiert, vertikal scrollbar sync) */}
        <div ref={labelBodyRef} style={{ width: labelW, flexShrink: 0, overflowY: "hidden", borderRight: "1px solid #d4dce4", position: "relative" }}>
          {sorted.map(({ task: t, origIdx }, i) => {
            ...
            const hatSel = selGuids?.size ? t.objektGuids.some(g => selGuids!.has(g)) : false;
        ...
        {/* Body rechts: Chart (scrollbar horizontal + vertikal) */}
        <div ref={bodyScrollRef} onScroll={() => { syncScroll(); syncVScroll(); }}
          style={{ flex: 1, overflow: "auto" }}>
          <svg width={chartW} height={bodyH} style={{ display: "block" }}>
            {/* Monats-Linien */}
            {rawMarkers.map((m, i) => (
              <line key={`ml${i}`} x1={m.x} y1={0} x2={m.x} y2={bodyH} stroke="#d4dce4" strokeWidth={0.6} />
            ))}
