// KalenderManager.tsx — Fenster zur Pflege des Arbeitstage-Kalenders (Feiertage) einer Simulation.
// Wochenenden sind fix arbeitsfrei; hier werden zusätzliche Feiertage verwaltet, die Dauer-Anzeigen,
// Gantt-Schattierung und den MS-Project-Export kalenderbewusst statt rein kalendertag-basiert machen.
import { useState } from "react";
import type { SimProjekt } from "../types";
import type { Feiertag } from "./kalenderHelpers";
...
export default function KalenderManager({ sim, updateSim, onClose }: Props) {
  ...
  function speichern(neu: Feiertag[]) {
    const sortiert = [...neu].sort((a, b) => a.datum.localeCompare(b.datum));
    setFeiertage(sortiert);
    updateSim({ ...sim, kalender: { feiertage: sortiert } });
  }

  function zeileAendern(idx: number, feld: "datum" | "name", wert: string) {
    speichern(feiertage.map((f, i) => i === idx ? { ...f, [feld]: wert } : f));
  }

  function zeileEntfernen(idx: number) {
    speichern(feiertage.filter((_, i) => i !== idx));
  }
  ...
  function zeileHinzufuegen() {
    ...
    speichern([...feiertage, { datum: iso, name: "" }]);
  }
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}
      ...
      <div style={{ background: "#fff", width: 460, maxWidth: "92vw", maxHeight: "85vh", overflowY: "auto",
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", borderBottom: "1px solid var(--tc-border-light)" }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: "var(--tc-text)" }}>Kalender / Feiertage</div>
        ...
        <div style={{ padding: "14px 18px" }}>
          <div style={{ fontSize: 11, color: "var(--tc-text-2)", lineHeight: 1.5, marginBottom: 12 }}>
            Wochenenden gelten immer als arbeitsfrei. Zusätzliche Feiertage hier eintragen — sie wirken sich auf
            angezeigte Dauern, die Gantt-Schattierung und den MS-Project-Export dieser Simulation aus.
          </div>
